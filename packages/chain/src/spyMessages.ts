...
import { SecurityCode, AgentDetails } from "./message";
...
export class SpyMessages extends RuntimeModule<Record<string, never>> {
  ...
  @runtimeMethod()
  public addMessage(agentId: UInt64, securityCode: SecurityCode, messageContent: CircuitString, messageNo: UInt64): void {
    ...
    //@ts-ignore
    assert(messageContent.length().lessThanOrEqual(Field(12)))
